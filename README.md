...
模型安装之前，需要提前编译好模型的依赖库，详情见[模式中台环境搭建](https://github.com/XiaoLinhong/envs)。
...
本文档介绍的是我在模式安装领域建立的**认知系统**，读完整个文档，也不一定在编译的时候一帆风顺（环境是多变的），但是希望能够帮助大家在这个领域去构建一个自己**认知系统**，在需要处理相应工作时，因该基于已经构建的认知系统去梳理处理思路，遇到问题的时候，应该基于认知系统去解决问题（而不是瞎搞、瞎试）。当你的认知系统无法解决该领域的问题时，或者无法高效地解决该问题时（我们都通常喜欢待着自己的舒服区，也可能是学习成本的原因，只要能解决，不愿意去改变，不过希望大家能够能多的勇气、决心和魄力去探索！），就是它升级、扩充的时候，也是你成长的时候（应该感到开心、而不是沮丧）。
